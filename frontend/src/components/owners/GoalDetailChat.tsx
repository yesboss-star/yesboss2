...
export default function GoalDetailChat({
  ...
  const handleAttachFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploadUserMsg = `📎 Uploaded file **${file.name}**. ${data.message || ""}`;
      setMessages((prev) => [...prev, { role: "user", content: uploadUserMsg }]);
      const result = await goalChat(goalId, `I uploaded a file: ${file.name}. ${data.message ? "The system says: " + data.message : "Please analyze this file and help me refine the goal based on it."}`);
