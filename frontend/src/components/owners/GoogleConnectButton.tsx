...
export default function GoogleConnectButton() {
  const { connected, email, loading, connecting, checkStatus, connect, disconnect } = useGoogleStore();
  ...
  const handleDisconnect = async () => {
    if (window.confirm("Disconnect Google integration? Tasks and calendar will stop syncing.")) {
      await disconnect();
    }
  };
  ...
  return (
    ...
    >
      ...
      {(connecting || localConnecting) ? "Connecting..." : "Connect Gmail"}
    </button>
  );
}
